...
10. **Save Task List:** Save the generated document in the `/작업/진행중/` directory with the filename `tasks-[feature-name].md`, where `[feature-name]` describes the main feature or task being implemented (e.g., if the request was about user profile editing, the output is `tasks-user-profile-editing.md`).
11. **Complete Task List:** When all tasks are completed (all checkboxes marked as `[x]`), move the file to `/작업/완료/`.
