...
본 사업은 **BC카드의 결제 인프라**와 **웨이브릿지(이하 WB)의 가상자산사업자(VASP) 인프라**를 결합하여, BC페이북 1,100만 회원 대상 스테이블코인 결제 서비스를 구축하는 것을 목표로 합니다.

| 구분 | BC카드 | 웨이브릿지 |
|------|--------|-----------|
| 전략적 의의 | 차세대 결제 인프라 확보, 디지털 자산 결제 선점 | VASP 기반 B2B2C 서비스 모델 확장 |
| 핵심 자산 | 페이북 1,100만 회원, 전국 가맹점 네트워크 | VASP 라이선스, MPC 월렛, 커스터디, 온오프램프 |
| 역할 | 고객 접점 및 가맹점 인프라 (프론트) | 가상자산 인프라 전체 (백엔드) |
...
- **BC카드**: 페이북 앱 내 사용자 접점(UI/UX) 제공, 기존 가맹점 네트워크를 스테이블코인 결제 인프라로 확장, BC카드 정산망을 통한 가맹점 원화 정산
- **웨이브릿지**: MPC 기반 비수탁 지갑 운영, 수탁형 커스터디 서비스, 원화-스테이블코인 전환(온오프램프), 전 구간 컴플라이언스(AML, 트래블룰) 수행

**자산 소유권·보관 구조**

| 구성요소 | 소유권 | 보관·관리 주체 | 핵심 포인트 |
|---------|--------|--------------|------------|
| 페이북 월렛 (비수탁) | 사용자 본인 | 사용자 직접 관리 (MPC 키 보유) | BC카드·WB 소유 아님 |
| WB 커스터디 (수탁) | BC카드 명의 | WB가 VASP로서 위탁 보관 | BC카드 소유이나 직접 보관하지 않음 |
| 결제 정산 흐름 | — | WB 경유 | BC카드가 가상자산을 실보유하지 않는 구조 |

**핵심 기술 요소**

- **MPC 2-of-3 비수탁 지갑**: 사용자·BC카드·WB 3자가 키 샤드를 분산 보유하며, 2자 이상 합의 시에만 트랜잭션이 성립합니다. 어떤 단일 주체도 독립적으로 자산을 이동할 수 없으므로 비수탁(non-custodial)으로 분류됩니다.
...
- **금가분리 원칙 충족**: BC카드는 서비스 UI를 제공할 뿐 가상자산을 직접 보유·취급하지 않습니다. 모든 가상자산의 보관·이동·전환은 VASP인 WB를 통해 수행됩니다.
...
- **트래블룰·AML 대응**: 모든 외부 전송에 대해 WB의 MPC 서명 전 컴플라이언스 게이트가 작동합니다. WB가 온체인 AML 분석, 제재 리스트 스크리닝, 트래블룰 정보 교환을 사전 수행하며, 미승인 시 트랜잭션 자체가 성립하지 않는 구조입니다.
...
| 수탁 수수료 | 수탁 잔액 기반 연율 | BC카드 명의 커스터디 자산 보관 |

**시장 기회**: BC카드의 연간 카드결제 처리 규모는 약 219조원입니다. 스테이블코인 결제가 이 중 극히 일부(0.1%)만 전환하더라도 약 2,190억원 규모의 거래량이 발생하며, 이에 수반되는 온오프램프 전환 및 수탁 수수료가 WB의 안정적 수익원이 됩니다.

**운영 효율성**: 네팅(상계) 구조를 통해 온램프(고객 충전)와 오프램프(가맹점 정산)를 일일 단위로 상계 처리함으로써, 실제 원화-스테이블코인 전환량을 최소화하고 가스비 및 전환 비용을 절감합니다.
...
gantt
    ...
    section Phase 1
    구조 확정 및 NDA       :2026-Q2, 2026-Q2
    section Phase 2
    PoC (실증)             :2026-Q3, 2026-Q3
    section Phase 3
    서비스 상용화           :2026-Q4, 2026-Q4
    section Phase 4
    AI 에이전트 확장        :2027-Q1, 2027-Q4
```
...
- 디지털 자산에 친숙한 MZ세대 고객 유입
- 페이북 플랫폼의 서비스 다양화 및 사용자 체류 시간 확대

**웨이브릿지의 사업 목표**

- VASP 라이선스 기반 B2B2C 서비스 모델 확장
- 대규모 리테일 결제 인프라와의 연동을 통한 거래량 확보
- 커스터디·온오프램프·MPC 월렛 등 서비스 라인업의 상용화 검증

### 1.2 단기 과제 (2026 Q3~Q4)

| 과제 | 내용 | 담당 |
|------|------|------|
| 페이북 월렛 PoC | MPC 기반 비수탁 지갑의 BC페이북 앱 내 임베딩 실증 | WB(월렛) + BC(앱) |
| WB 커스터디 연동 | BC카드 명의 수탁 지갑 생성 및 자산 보관 테스트 | WB |
| 온오프램프 테스트 | 원화-스테이블코인 전환 및 네팅 프로세스 검증 | WB |
| 가맹점 결제 실증 | 소규모 가맹점 그룹 대상 QR 결제 테스트 | BC(가맹점) + WB(결제) |
...
현행 금융규제 체계에서는 금융회사와 가상자산 취급 간의 분리(이하 "금가분리 원칙")가 적용되고 있습니다. 이에 따라 본 사업에서 BC카드는 **고객 접점 및 결제 인프라를 제공하되, 가상자산의 직접 보유·취급은 수행하지 않는** 구조로 참여합니다. 가상자산의 보관·이동·전환에 관한 모든 업무는 VASP인 웨이브릿지가 담당합니다.

### 2.2 BC카드가 제공하는 서비스

**B2C 고객 접점**

BC카드는 페이북 앱 내에 "페이북 월렛" 서비스를 탑재합니다. 사용자는 기존 페이북 앱에서 스테이블코인 충전, 잔액 조회, 결제, 전송, 환전 기능을 이용할 수 있습니다. 지갑의 기술적 운영(MPC 서명, 블록체인 트랜잭션 처리)은 WB가 수행하며, BC카드는 사용자 인터페이스와 고객 인증을 담당합니다.
...
| 지갑 운영 | 키 샤드 #2 보유 (인증·정책) | MPC 서버 운영, 키 샤드 #3 보유 |
| 자산 보관 | — | 커스터디 서비스 (수탁) |
| 원화 전환 | — | 온오프램프 수행 |
...
flowchart LR
    subgraph OWNERSHIP["자산 소유권 구조"]
        direction TB
        A["사용자 (B2C)"] -->|"본인 소유<br>비수탁 지갑"| PW["페이북 월렛"]
...
본 구조에서 BC카드는 가상자산의 **법적 소유자**(커스터디 계정 명의)이나, 실제 보관·관리는 WB에 위탁합니다. 이를 통해 BC카드가 가상자산을 직접 보유(실보유)하지 않는 구조를 유지하면서도, 서비스 운영에 필요한 자산 통제권을 확보합니다.
...
페이북 월렛은 사용자가 스테이블코인을 직접 보유하고, 이를 가맹점 결제·외부 전송·환전 등에 활용할 수 있도록 하는 디지털 지갑입니다. 기술적으로는 MPC(Multi-Party Computation) 기반의 비수탁형 구조를 채택하여, 어떤 단일 주체도 사용자의 자산에 독립적으로 접근할 수 없도록 설계되어 있습니다.
...
기존의 단일 개인키 방식과 비교하면 다음과 같은 이점이 있습니다.
...
| 샤드 #3 | WB MPC 서버 | 컴플라이언스 검증 (AML, 트래블룰, 이상거래) | **불가** |
...
2. **BC카드·WB의 독립적 접근 불가**: BC카드(샤드 #2)와 WB(샤드 #3)가 합의하더라도, 사용자(샤드 #1) 없이는 자산 이동이 불가합니다. 2-of-3 구조에서 사용자의 참여가 필수적입니다.
...
**대응 방향**: 서비스 출시 전 FIU(금융정보분석원) 사전 상담을 통해 비수탁 분류의 유효성을 확인하며, 수탁 판정 시를 대비한 대체 구조(WB가 전체 사용자 KYC를 수행하는 수탁 모델)도 병행 검토합니다. 상세 분석은 섹션 6에서 다룹니다.
...
내부 전송(페이북 월렛 → WB 커스터디)은 양측 모두 WB 통제 하에 있으므로, 별도의 외부 AML 검증 없이 처리됩니다.
...
sequenceDiagram
    ...
    participant S3 as WB MPC 서버<br>(샤드 #3)
...
본 프로세스의 핵심은 **Step 3의 컴플라이언스 게이트**입니다. WB MPC 서버가 모든 트랜잭션에 대해 AML/CFT 검증을 수행하고, 미승인 시 샤드 #3 서명을 거부함으로써 트랜잭션 자체를 차단합니다. 이는 비수탁 지갑에서도 VASP 수준의 컴플라이언스를 실현하는 구조적 장치입니다.
...
**장점**: 가맹점이 스테이블코인을 자율적으로 관리할 수 있으며, WB에 대한 보관 위탁이 불필요합니다.

**한계**:

- **원화 정산 문제**: 가맹점이 스테이블코인을 원화로 전환하려면 직접 오프램프를 수행해야 합니다. 이를 위해서는 각 가맹점이 WB(또는 다른 VASP)에 개별 온보딩해야 하며, 소규모 가맹점의 경우 현실적으로 어렵습니다.
...
- **자동 원화 정산**: WB 커스터디에 수신된 스테이블코인을 자동으로 원화 전환(오프램프)하여 BC카드 정산망으로 전달합니다. 가맹점은 기존과 동일하게 원화로 정산받습니다.
- **가맹점 기술 부담 최소화**: 가맹점은 별도의 블록체인 기술 역량이 불필요합니다.
...
| 키 복구 | 24시간 이내 | 사용자 디바이스 분실 시, BC카드 본인 인증 + WB 키 재생성 |
| 앱 임베딩 | 네이티브 수준 UX | BC페이북 앱 SDK 연동, 웹뷰 vs 네이티브 모듈 선택 |
| 블록체인 네트워크 | 확정 필요 | Ethereum(높은 보안, 높은 가스비) vs Tron/Solana(낮은 비용, 높은 처리량) |
...
| 대상 | BC카드 (B2B 서비스) |
| 서비스 유형 | 수탁 커스터디 + 온오프램프 + 지갑 전송 + 가맹점 정산 |
| 법적 기반 | 특금법상 VASP 신고 기반 가상자산 보관·관리 서비스 |

웨이브릿지 프라임은 BC카드에 제공하는 B2B 서비스 패키지로서, 다음 4가지 핵심 서비스를 포함합니다.
...
| 다중 서명 | 커스터디 자산 출금 시 WB 내부 복수 승인자의 다중 서명 필요 |
...
네팅은 일정 기간(기본: 일일) 동안 발생한 온램프 수요와 오프램프 수요를 상계하여, **차액분만 실제 전환**하는 구조입니다.
...
온/오프램프 수수료는 비대칭 설정이 가능합니다. 예를 들어, USDC의 경우 Circle을 통한 민팅(온램프)은 수수료가 없거나 매우 낮으나, 소각(오프램프)은 수수료가 발생할 수 있어 이를 반영합니다. 구체적인 요율은 양사 간 별도 협의를 통해 확정합니다.

### 4.4 고객 지갑 전송 서비스

#### 4.4.1 WB 커스터디 → 페이북 월렛 (충전 배포)

사용자가 충전을 요청하면, WB 커스터디에서 개별 사용자의 페이북 월렛으로 스테이블코인을 온체인 전송합니다. 이 구간은 **인플로우(Inflow)**에 해당하며, 서비스 전체에서 운영 공수가 가장 높은 구간입니다.
...
각 전송 건에 대해 **건별 전송 수수료**가 부과되며, 이는 온체인 가스비 및 WB 운영 비용을 포함합니다.
...
아웃플로우는 페이북 월렛 → WB 수탁 지갑 간의 전송이므로, 양측 모두 WB 통제 하에 있어 외부 AML 이슈가 최소화됩니다. 또한, 가맹점 결제 대금은 배치 집계 후 일괄 오프램프 처리가 가능하여 운영이 단순합니다.
...
| 가스비 | 상대적 낮음 | **상대적 높음** |
| 운영 공수 | 단순 | **복잡** |
| AML 부담 | 최소 (내부 전송) | 최소 (내부 전송) |

인플로우(충전 배포)는 **대량의 개별 사용자 지갑에 각각 온체인 트랜잭션을 발생**시켜야 하므로, 가스비·처리 시간·트랜잭션 모니터링 측면에서 운영 공수가 큽니다. 이러한 비대칭성이 수수료 구조에 반영됩니다(건별 전송 수수료가 별도 책정되는 이유).
...
- **온보딩 절차**: 가맹점 사업자 정보 확인(KYB), WB 이용약관 동의, 수탁지갑 생성
- **BC카드 가맹점 등록과의 연계**: BC카드 가맹점 등록 프로세스에 WB KYB를 포함하여 일원화 처리 가능
...
- **WB 수탁지갑 내 보유**: 오프램프를 수행하지 않고 WB 수탁지갑에 스테이블코인을 보유
...
| 구간 | 발신 | 수신 | AML 적용 | 근거 |
|------|------|------|---------|------|
| 내부 | 페이북 월렛 | WB 커스터디 | 불필요 | 양측 모두 WB 통제 하, 사용자 신원 확인 완료 |
| 내부 | WB 커스터디 | 페이북 월렛 | 불필요 | 양측 모두 WB 통제 하 |
...
4. **판정 및 조치**:
   ...
   - **고위험**: 수신 보류, WB 컴플라이언스 팀 수동 검토 후 승인/반환 결정
...
이 구조는 비수탁 지갑에서도 VASP 수준의 자금세탁방지 의무를 이행할 수 있도록 하는 핵심 메커니즘입니다.
...
BC카드에 대한 AML 리포트 제공 범위에 관해서는 온체인 분석 도구 제공사의 2차 제공(재배포) 라이선스 이슈가 있으므로, 별도 확인이 필요합니다.

### 5.5 WB의 AML 서비스 범위 정의
...
BC카드는 이미 여전법에 의거한 고객 확인(KYC) 절차를 운영하고 있습니다. WB가 BC카드의 KYC 결과를 수용(제3자 KYC 의존)할 수 있다면, 사용자의 이중 온보딩을 방지할 수 있습니다. 다만, FATF Recommendation 17의 요건을 충족하기 위해서는 WB가 BC카드의 KYC 절차 적정성을 독립적으로 평가해야 합니다.

### 6.3 BC카드의 서비스 범위와 금가분리

여전법에 의거한 카드사인 BC카드가 스테이블코인 결제 UI를 제공하는 행위에 대해서는 다음과 같은 규제적 해석이 필요합니다.

- **현행 체계**: BC카드는 결제 UI 및 가맹점 네트워크만 제공하고, 가상자산의 보관·이동·전환은 수행하지 않으므로 금가분리 원칙에 저촉되지 않는 것으로 판단됩니다.
- **디지털자산기본법 시행 후**: 동 법에서 스테이블코인의 "지급이전" 기능을 VASP 업권 내로 정의할 경우, BC카드의 역할이 보다 명확히 구분됩니다. 다만, 금융회사의 가상자산 서비스 제공 범위에 대한 세부 규정 확인이 필요합니다.

### 6.4 트래블룰 적용 구조

비수탁 지갑(페이북 월렛)에서 외부 수탁지갑으로 전송 시, "발신 VASP"의 존재 여부가 쟁점입니다.

- **문제**: 페이북 월렛은 비수탁이므로, 형식상 발신 측에 VASP가 존재하지 않습니다. 수신 VASP가 발신 VASP에게 트래블룰 정보를 요청할 대상이 없습니다.
- **대응 구조**: WB가 MPC 서명 과정에서 발신측 정보를 보유하고 있으므로, 실질적으로 트래블룰 정보 교환의 역할을 수행합니다. WB의 MPC 컴플라이언스 게이트에서 수신 VASP 식별 및 정보 교환을 사전 수행한 후 서명을 완성하는 방식으로, 트래블룰 의무를 이행합니다.
...
회신 예정일은 2026. 02. 20.이며, 검토 결과는 본 문서의 차기 버전(v0.2)에 반영할 예정입니다.
...
| 건별 전송 수수료 | 건당 정액 또는 정률 | WB 커스터디 → 페이북 월렛 전송 시 |
| 수탁 수수료 | 수탁 잔액 기반 연율 | BC카드 명의 커스터디 보관 |
