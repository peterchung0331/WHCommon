...
```mermaid
flowchart TB
    subgraph REQ["BC카드 스테이블코인 서비스에 필요한 VASP 역량"]
        direction LR
...
| 온오프램프 | 자체 거래를 통한 전환 가능 |
| B2B 서비스 대응 | **미흡** — 리테일 트레이딩 중심 사업 모델 |
| 이해충돌 리스크 | **높음** — 자체 거래소와 BC카드 서비스 간 이해충돌 가능 |

**한계**: 거래소의 본업은 리테일 고객 대상 가상자산 매매이며, 카드사 규모의 B2B 커스터디·온오프램프 서비스를 제공하는 사업 구조를 갖추고 있지 않습니다. 또한 거래소가 BC카드의 결제 정산 자산을 보관할 경우, 자체 거래소 사업과의 이해충돌 문제가 발생할 수 있습니다. 독립적인 수탁자로서의 중립성이 부족합니다.
...
커스터디·온오프램프·컴플라이언스가 하나의 사업자 내에서 운영되므로, 사업자 간 자산 이동에 따른 추가 온체인 트랜잭션·가스비·시간 지연이 발생하지 않습니다. 네팅(상계)도 단일 사업자 내에서 처리되어 운영 효율성이 극대화됩니다.
...
| 보안 요소 | 적용 내용 |
|----------|----------|
| 콜드/핫 월렛 분리 | 대부분 오프라인 콜드 월렛, 운영 최소 금액만 핫 월렛 |
...
- 네팅 **비대상**: 온체인 전송 (WB 커스터디 → 페이북 월렛). 전송은 건별 실제 트랜잭션 필요.
...
```mermaid
flowchart TB
    subgraph SYSTEM["전체 전송 구간 AML 적용 구분"]
        direction LR
...
- **디지털자산기본법 시행 후**: 비수탁 지갑 구축자의 법적 지위에 대한 세부 규정 확인이 필요합니다.
...
구체적인 요율은 양사 간 별도 협의를 통해 확정하며, Phase 1 종료 시점까지 합의를 목표로 합니다.
...
| 네팅 | 매수(온램프)와 매도(오프램프) 수요를 상계하여 차액분만 실제 전환하는 구조 |
...
| 디지털자산기본법 (입법 예정) | 스테이블코인 지급이전, VASP 업권 재정의 |
