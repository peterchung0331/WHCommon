...
- [x] 1.0 [PARALLEL GROUP: schema-changes] 스키마 변경 (마이그레이션 SQL)
  ...
  - [x] 1.4 로컬/오라클 DB에 마이그레이션 적용 및 검증
    - [x] 로컬 DB 마이그레이션 완료 (12개 컬럼, 4개 인덱스)
    - [x] 로컬 DB에 sales_stage enum 값 추가 (6개 신규 값)
...
- [x] 7.0 DB 마이그레이션
  - [x] 7.1 로컬 DB 마이그레이션 완료
  - [x] 7.2 오라클 DB 마이그레이션 완료
    - 12개 컬럼 추가 완료
    - 4개 인덱스 생성 완료

- [x] 8.0 데이터 임포트 완료
  - [x] 8.1 CSV 파일 위치 확인 (Windows Downloads 폴더)
  - [x] 8.2 임포트 스크립트 실행 (로컬 DB)
    - 34개 고객 데이터 생성 완료
    - 18개 중복 데이터 스킵
    - 0개 오류
  - [x] 8.3 데이터 검증 완료
