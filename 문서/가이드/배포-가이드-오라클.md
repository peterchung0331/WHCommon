...
- ❌ `password authentication failed for user "workhub"` (비밀번호 불일치)

**비밀번호 인증 실패 시 해결 방법**:
```bash
# 오라클 서버에서 workhub 사용자 비밀번호 재설정
sudo -u postgres psql -c "ALTER USER workhub WITH PASSWORD 'Wnsgh22dml2026';"

# 비밀번호 테스트
PGPASSWORD='Wnsgh22dml2026' psql -U workhub -d testagent -h localhost -c 'SELECT 1;'

# 컨테이너 재시작하여 새 비밀번호 적용
docker restart hwtestagent-api wbhubmanager-staging wbsaleshub-staging
```
...
### 원칙
- ✅ `.env.staging`, `.env.prd` 파일만 사용
- ✅ Docker Compose로 환경변수 자동 로드
- ❌ Doppler 사용 금지 (보안 이슈 및 복잡도 증가)
...
1. `.env.staging` 파일 준비 (Git 제외)
2. `docker-compose.staging.yml` 확인
3. 배포 스크립트 실행: `./deploy-staging.sh`

### 검증

```bash
# 컨테이너에서 JWT 키 길이 확인
docker compose exec hubmanager-staging \
  node -e "console.log('JWT_PRIVATE_KEY length:', process.env.JWT_PRIVATE_KEY?.length || 0)"
