...
- **[MCP-설정-가이드.md](./MCP-설정-가이드.md): Claude Code MCP 서버 설정 (필수)**
- [MCP-설치-완료-안내.md](./MCP-설치-완료-안내.md): MCP 설치 완료 후 다음 단계
...
#### 2.6.3 Claude Context Local 설치 (선택)
...
# 권한: "Read access to contents of all public gated repos you can access" 체크
...
# https://huggingface.co/google/embeddinggemma-300m 접속
# "Accept license and access repository" 버튼 클릭

# 4. 모델 다운로드 (권한 승인 후, 약 1.3GB)
...
**상세 가이드**: [grep-대안-도구-사용법.md](./grep-대안-도구-사용법.md)
...
기존 Bash 명령어를 대체하는 빠르고 강력한 모던 CLI 도구들입니다. Claude Code와 함께 사용하면 개발 효율이 크게 향상됩니다.

#### 2.7.1 핵심 도구 요약

| 기존 명령 | 모던 대안 | 특징 | Claude Code 적합성 |
|----------|----------|------|-------------------|
| `cat` | **bat** | 구문 하이라이팅, 라인 번호, Git 통합 | ⭐⭐⭐⭐⭐ |
| `find` | **fd** | 30배 빠름, .gitignore 인식, 간단한 문법 | ⭐⭐⭐⭐⭐ |
| `ls` | **eza** | Git 상태 표시, 아이콘, 트리뷰 | ⭐⭐⭐⭐ |
| `git diff` | **delta** | 구문 하이라이팅, side-by-side diff | ⭐⭐⭐⭐ |
| `cd` | **zoxide** | 디렉토리 학습, 퍼지 매칭 | ⭐⭐⭐ |

#### 2.7.2 bat 설치 (cat 대체)

구문 하이라이팅과 Git 통합을 지원하는 `cat` 대체 도구입니다.

```bash
# Ubuntu/WSL
sudo apt install bat

# 설치 확인 (Ubuntu에서는 batcat으로 실행)
batcat --version
# 출력: bat 0.19.0
...
# 특정 라인 범위만 보기
batcat src/index.ts -r 10:20

# 라인 번호 없이 출력
batcat -p src/index.ts
```

#### 2.7.3 fd 설치 (find 대체)

30배 빠르고 사용하기 쉬운 `find` 대체 도구입니다.

```bash
# Ubuntu/WSL
sudo apt install fd-find

# 설치 확인 (Ubuntu에서는 fdfind로 실행)
fdfind --version
# 출력: fd 8.3.1
...
Git 통합과 아이콘을 지원하는 `ls` 대체 도구입니다.

```bash
# Ubuntu/WSL (외부 저장소 추가 필요)
...
# 설치 확인
eza --version
# 출력: eza - A modern, maintained replacement for ls v0.23.4

# alias 설정 (선택사항)
echo 'alias ls="eza"' >> ~/.bashrc
echo 'alias ll="eza -la"' >> ~/.bashrc
echo 'alias tree="eza --tree"' >> ~/.bashrc
source ~/.bashrc
```

**주요 기능**:
- Git 상태 통합 표시 (M: 수정, N: 새 파일)
- 파일 아이콘 표시
- 트리뷰 기능
...
#### 2.7.5 delta 설치 (git diff 대체)

구문 하이라이팅과 side-by-side diff를 지원하는 Git diff 도구입니다.

```bash
# Ubuntu/WSL (GitHub에서 직접 다운로드)
...
#### 2.7.6 zoxide 설치 (cd 대체)

자주 방문하는 디렉토리를 학습하는 스마트 `cd` 도구입니다.
...
```bash
# 학습된 디렉토리로 이동
z hub        # ~/GitHub/WBHubManager로 이동 (학습 후)
z sales      # ~/GitHub/WBSalesHub로 이동
...
**참고**: zoxide는 사용할수록 학습하므로, 초기에는 전체 경로를 입력해야 합니다.

#### 2.7.7 모던 CLI 도구 요약 테이블

| 도구 | Ubuntu 명령어 | alias 권장 | Claude Code 활용 |
|------|-------------|------------|-----------------|
| bat | `batcat` | `alias bat="batcat"` | 파일 읽기 시 구문 하이라이팅 |
| fd | `fdfind` | `alias fd="fdfind"` | 빠른 파일 검색 |
| eza | `eza` | `alias ls="eza"` | 디렉토리 구조 파악 |
| delta | `delta` | Git에 자동 연동 | diff 가독성 향상 |
| zoxide | `z` | 기본 제공 | 빠른 디렉토리 이동 |
...
WHCommon은 별도의 Git 저장소이므로 `~/GitHub/WHCommon/`에 위치합니다.

```bash
mkdir -p ~/GitHub/WHCommon/keys
```

### 6.2 JWT 키 페어

**위치**:
- `~/GitHub/WHCommon/keys/jwt_private.pem`
- `~/GitHub/WHCommon/keys/jwt_public.pem`

**획득 방법**:
1. **팀 리더에게 요청** (권장): 기존 키를 공유받음
2. **신규 생성** (개발 전용):
   ```bash
   cd ~/GitHub/WHCommon/keys
...
**권한 설정**:
```bash
chmod 600 ~/GitHub/WHCommon/keys/jwt_private.pem
chmod 644 ~/GitHub/WHCommon/keys/jwt_public.pem
```

### 6.3 Doppler 토큰 파일

**위치**: `~/GitHub/WHCommon/env.doppler`
...
```bash
nano ~/GitHub/WHCommon/env.doppler
...
**위치**:
- `~/GitHub/WHCommon/keys/oracle-cloud.key`
...
```bash
# WHCommon/keys에 저장
cp /path/to/received/oracle-cloud.key ~/GitHub/WHCommon/keys/

# WSL SSH 디렉토리에 복사
cp ~/GitHub/WHCommon/keys/oracle-cloud.key ~/.ssh/

# 권한 설정
chmod 600 ~/GitHub/WHCommon/keys/oracle-cloud.key
...
```bash
cat ~/GitHub/WHCommon/env.doppler
```
...
`~/GitHub/WHCommon/env.doppler` 파일에서 각 프로젝트의 토큰을 확인한 후 실행합니다.

#### WBHubManager

```bash
cd ~/GitHub/WBHubManager
DOPPLER_TOKEN=<HUBMANAGER_DEV_토큰> doppler secrets download --no-file --format env > .env.local
# ✅ .env.local 파일 생성됨
```
...
DOPPLER_TOKEN=<SALESHUB_DEV_토큰> doppler secrets download --no-file --format env > .env.local
```
...
```bash
cd ~/GitHub/WBFinHub
DOPPLER_TOKEN=<FINHUB_DEV_토큰> doppler secrets download --no-file --format env > .env.local
```
...
DOPPLER_TOKEN=<ONBOARDINGHUB_DEV_토큰> doppler secrets download --no-file --format env > .env.local
```

**참고**: 토큰은 `~/GitHub/WHCommon/env.doppler` 파일에 `DOPPLER_TOKEN_<허브명>_DEV=dp.st.dev.xxx` 형식으로 저장되어 있습니다.
...
```bash
# PostgreSQL 컨테이너 접속
docker exec -it workhub-postgres psql -U postgres
...
### 8.2 의존성 설치 및 데이터베이스 초기화

대부분의 허브는 raw SQL 기반으로, 서버 최초 실행 시 테이블이 자동 생성됩니다.
WBOnboardingHub만 Prisma를 사용합니다.

#### WBHubManager (raw SQL)

```bash
cd ~/GitHub/WBHubManager
npm install
npm run dev  # 서버 시작 시 DB 테이블 자동 생성
```
...
Docker 환경에서 WSL IP를 수동으로 확인하고 `.env.local`에 반영합니다:
...
`.env.local` 파일에서 다음 항목을 업데이트합니다:
```bash
WSL_HOST_IP=<확인된_IP>
DOCKER_HOST_URL=http://<확인된_IP>
```
...
설치된 Git Hook은 `sync-doppler.sh` 스크립트를 통해 Doppler와 환경변수를 동기화합니다.
현재 `sync-doppler.sh`가 없는 경우 hook은 자동으로 skip됩니다 (경고 메시지 출력 후 정상 진행).
...
필요시 Doppler CLI로 수동 동기화할 수 있습니다:

```bash
# Doppler Development → .env.local 다운로드
cd ~/GitHub/WBHubManager
DOPPLER_TOKEN=<토큰> doppler secrets download --no-file --format env > .env.local
```

**참고**: 토큰은 `~/GitHub/WHCommon/env.doppler` 파일에서 확인합니다.
...
# 데이터베이스 접속 테스트
docker exec -it workhub-postgres psql -U postgres -l
```
...
# .env.local에서 WSL_HOST_IP, DOCKER_HOST_URL 수동 업데이트
nano ~/GitHub/WBHubManager/.env.local
# WSL_HOST_IP=<새로운_IP>
# DOCKER_HOST_URL=http://<새로운_IP>
...
### 13.6 데이터베이스 테이블 생성 실패

**증상**: 서버 시작 시 테이블 생성 에러 (HubManager, SalesHub, FinHub)
...
# 서버 재시작으로 자동 마이그레이션 재시도
npm run dev
```

**WBOnboardingHub (Prisma) 마이그레이션 실패 시**:
```bash
cd ~/GitHub/WBOnboardingHub
...
- [ ] ripgrep 설치 (`rg --version` 확인)
- [ ] ast-grep 설치 (`sg --version` 확인)
- [ ] Claude Context Local 설치 (선택)
- [ ] bat 설치 (`batcat --version` 확인)
- [ ] fd 설치 (`fdfind --version` 확인)
- [ ] eza 설치 (`eza --version` 확인)
- [ ] delta 설치 (`delta --version` 확인)
- [ ] zoxide 설치 (`zoxide --version` 확인)
...
- [ ] WSL IP 확인 및 `.env.local`에 반영

### 데이터베이스
- [ ] Docker PostgreSQL 실행
- [ ] 데이터베이스 생성 (hubmanager, saleshub, finhub 등)
- [ ] `npm install` 완료 (각 허브)
- [ ] 서버 실행 시 DB 테이블 자동 생성 확인 (HubManager, SalesHub, FinHub)
- [ ] WBOnboardingHub: Prisma 마이그레이션 완료 (`npx prisma migrate dev`)
