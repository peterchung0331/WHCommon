...
- 빌드 시간, 로그 시간, 작업 기록 날짜 등 모든 시간은 한국시간 기준
...
#### 필수 동기화 대상
- ✅ `기획/진행중/` - 진행중인 PRD 파일 (다른 PC에서 참조 가능하도록)
- ✅ `기획/완료/` - 완료된 PRD 파일
- ✅ `작업/진행중/` - 진행중인 Task 파일 (다른 PC에서 재개 가능하도록)
- ✅ `작업/완료/` - 완료된 Task 파일
- ✅ `작업기록/진행중/` - 진행중 작업 로그
- ✅ `작업기록/완료/` - 완료된 작업 로그
- ✅ `작업기록/보류/` - 보류된 작업 로그
- ✅ `claude-context.md` - 컨텍스트 설정 파일
- ✅ `규칙/` - 실행 규칙 파일

#### 동기화 시점
1. **PRD/Task 파일 생성 시**: 즉시 커밋 및 푸시
2. **작업 진행 중**: 주요 마일스톤 완료 시 커밋 및 푸시
3. **작업 완료 시**: 파일 이동 후 즉시 커밋 및 푸시
4. **컨텍스트 파일 수정 시**: 즉시 커밋 및 푸시

#### 커밋 메시지 규칙
```bash
# PRD 파일 추가/수정
git commit -m "docs: [기능명] PRD 작성/수정"

# Task 파일 추가/수정
git commit -m "docs: [기능명] Task 진행 중/완료"

# 작업 로그 추가
git commit -m "docs: [날짜] 작업 로그 추가 - [설명]"

# 컨텍스트 파일 수정
git commit -m "docs: claude-context 업데이트 - [변경 내용]"
```

#### 다른 PC에서 작업 재개 시
```bash
# WHCommon 최신 상태로 업데이트
...
**플랜모드를 종료하고 실제 구현 작업을 시작할 때는 반드시 다음을 준수합니다:**

1. **실행_작업.md 필수 참조** (항상 첫 번째 단계)
   - 위치: `/home/peterchung/WHCommon/규칙/실행_작업.md`
   - ExitPlanMode 호출 직후, 첫 번째 파일 수정 전에 반드시 읽어야 함
   - 병렬 실행 그룹, 의존성 분석, 작업 순서 결정에 필수

2. **TodoWrite로 작업 추적**
   - 실행_작업.md 기반으로 병렬 실행 그룹 식별
   - 각 그룹을 TodoWrite에 등록하여 진행 상황 추적

3. **병렬 실행 우선**
   - 실행_작업.md에서 병렬 가능 여부 확인
   - 의존성 없는 작업은 동시 병렬 수행
   - 순차 의존성이 있는 경우에만 순서대로 진행
...
다음 상황에서는 **반드시** Sequential Thinking MCP를 사용하여 사고 과정을 구조화:

| 상황 | 예시 | Sequential Thinking 사용 |
|------|------|-------------------------|
| **원인 분석** | OOM, 빌드 실패, 버그 원인 추적 | ✅ 필수 |
| **아키텍처 설계** | 새 기능 설계, 시스템 구조 변경 | ✅ 필수 |
| **다단계 문제 해결** | 3단계 이상의 복잡한 해결 과정 | ✅ 필수 |
| **트레이드오프 분석** | 여러 옵션 비교, 장단점 분석 | ✅ 필수 |
| **Git 히스토리 분석** | 회귀 원인 추적, 변경점 분석 | ✅ 필수 |
| **성능 최적화** | 빌드 시간, 메모리, 용량 최적화 | ✅ 필수 |
| **단순 작업** | 환경변수 추가, 설정 변경, 오타 수정 | ❌ 불필요 |

**사용 방법**:
```
Claude는 복잡한 분석 시작 시 Sequential Thinking MCP를 호출하여:
1. 문제 정의 → 2. 가설 수립 → 3. 검증 → 4. 결론 도출
단계별로 사고 과정을 명시적으로 구조화
```
