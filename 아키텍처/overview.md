...
| **WBOnboardingHub** | `/home/peterchung/WBOnboardingHub` | 고객 온보딩, GCP Vision, Prisma ORM | 비활성 |
| **HWTestAgent** | `/home/peterchung/HWTestAgent` | 테스트 자동화, 에러 패턴 DB | 활성 |
| **WHCommon** | `/home/peterchung/WHCommon` | 공유 문서, 아키텍처, 스크립트 (배포 불가) | 문서 전용 |
...
├── server/modules/deal-transactions/ (딜 트랜잭션)
├── server/modules/customers/ (고객)
├── server/modules/sales-leads/ (영업 리드)
└── server/modules/integrations/ (Fireblocks)

WBOnboardingHub
├── packages/hub-auth (인증)
├── server/modules/ (비즈니스 로직)
└── prisma/ (Prisma ORM, GCP Vision)
```
